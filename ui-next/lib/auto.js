...
export function assembleSystemPrompt() {
  const parts = [
    ...
    [
      ...
      "Khi TƯỜNG THUẬT liệt kê dữ liệu có cấu trúc cho người dùng (file đã sửa, quality gate + kết quả,",
      "phân tích...) → dùng BẢNG Markdown (GFM) cho dễ đọc; UI render markdown. CHỈ áp dụng cho phần",
      "tường thuật — TUYỆT ĐỐI KHÔNG dùng cho PR body / commit / Jira comment / migration (giữ ĐÚNG template).",
      "",
      ...
      "   PHASE: lấy TỪ TICKET — là tag `[...]` ở ĐẦU summary ticket (vd summary `[PreUAT- MVP2-A] ...`",
      "   → phase `PreUAT-MVP2-A`; chuẩn hoá: bỏ khoảng trắng thừa bên trong tag). Nếu summary KHÔNG có",
      "   tag phase ở đầu (không lấy được) → dùng `UAT-MVP2-A`. TUYỆT ĐỐI không suy phase từ base branch.",
      ...
      "   CHECKLIST: only tick `[x]` a box if that item ACTUALLY passed — steps 5 (code quality)",
      "   and 6 (security `./semgrep-rules/scan.sh`) MUST have been run for real. NEVER tick the",
      "   quality/security boxes without having run them. If a gate fails, do NOT open the PR (see below).",
