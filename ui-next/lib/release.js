...
export function releaseSystemPrompt(nowStamp) {
  return [
    ...
    "mỗi ticket một người): DEV1 → người YÊU CẦU BUILD ticket đó (mapping ticket → người do user cấp, vd",
    "\"YenLTB yêu cầu build DEV1 REZIL-3001\"; accountId lấy bằng `lookupJiraAccountId`); STG → REPORTER của từng ticket",
    "(`getJiraIssue` field `reporter.accountId`). Dựng bảng `ticket → assignee` trước, đưa vào cùng confirm với label,",
    "tổng kết in bảng đã ghi thật. KHÔNG đoán: DEV1 mà thiếu/không rõ ai request ticket nào thì DỪNG và hỏi user.",
