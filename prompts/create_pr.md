...
## Steps
1. Ensure branch is pushed with upstream set — `git push -u origin HEAD` (never a bare `git push` on a fresh branch) — and rebased/up to date with `develop`.
...
3. Set base = `develop`. Title = `[<phase>] <SCREEN-CODE> | REZIL-XXXX - <summary>`. Phase LẤY TỪ TICKET — tag `[...]` ở đầu summary ticket (chuẩn hoá bỏ khoảng trắng thừa, vd `[PreUAT- MVP2-B]` → `PreUAT-MVP2-B`); nếu summary không có tag phase → để `UAT-MVP2-B`.
